...
func UpdateOrCreateCartItem(c *gin.Context) {
	var req model.UpdateOrCreateRequest
	err := c.BindJSON(&req)
	if err != nil {
		logrus.Error(err)
		c.JSON(400, gin.H{
	...
	err = module.UpdateOrCreate(c, &req)
	if err != nil {
		logrus.Error(err)
		c.JSON(500, gin.H{
			"error": "Internal Server Error",
	...
	c.JSON(200, gin.H{"message": "CartItem updated or created successfully"})
}
